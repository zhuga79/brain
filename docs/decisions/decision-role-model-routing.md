...
машинно-локальный), но при этом входил в системное дерево, которое уезжает в
публичный репозиторий. Один файл классифицирован как локальный и как общий
одновременно.
...
- **Публикация и федерация.** `config/routing.json` — системный путь: он живёт
  в публичном репозитории вместе с кодом. `.cli-mapping.sh` убирается из
  `RUNTIME_PATHS` федерации и из системного дерева, иначе локальная карта
  окажется в общем контракте.
...
знаний с человеческой курацией и markdown-валидацией, плюс слой данных,
которого нет в системном репозитории; конфиг исполнения там неотличим от знания.
