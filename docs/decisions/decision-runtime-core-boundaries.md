...
5. **`brain-validate` в контексте рабочей папки выдаёт неустранимые
   ...
   опознаёт folder-native рабочую папку (`is_folder_native_workspace`)
   раньше, чем требует `raw/`, `wiki/`, `tasks/active.md` — и возвращает один
   `INFO`-issue вместо ошибок по схеме, которой у такой папки нет и не должно
   быть. Проверка такой папки — `brain-workspace`, не `brain-validate`.

   **Опознание не может опираться на один маркер.** Признание папки рабочей
   отключает `validate_all` целиком — вместе с
   `validate_system_paths_not_restored` и `validate_installer_shadows`,
   которые держат границу корня данных (находка
   `data-root-stale-installer-reti` родительского ревью). Первая редакция
   этого решения проверяла только наличие `BRAIN.md`, и файл с таким именем,
   оказавшийся в корне данных, снимал четыре ERROR, включая installer-тень;
   воспроизведено при приёмке. Класс тот же, что закрыт 2026-08-16 в
   `tests/lib/sandbox-guard.sh`: контур опознавался признаком, который в
   живом дереве появляется сам. Поэтому опознание положительное и по
   свойству дерева — корень данных исключается по настроенному `BRAIN_PATH`
   и по собственной схеме (`tasks/active.md`, `wiki/`), которой у рабочей
   папки по контракту нет. Правило закреплено тестами
   `test_a_marker_in_the_data_root_does_not_make_it_a_workspace` и
   `test_false_for_the_configured_data_root_even_without_its_schema`.
