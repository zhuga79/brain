...
curl -s --noproxy '*' -X POST "http://127.0.0.1:$port/api/workspace/import?descriptor=/tmp/x" | python3 -c "
...
curl -s --noproxy '*' -H "X-Brain-Confirm: 1" -X POST "http://127.0.0.1:$port/api/workspace/import?descriptor=http://127.0.0.1/x" | python3 -c "
import sys,json; d=json.load(sys.stdin)
...
mkdir -p "$target_dir"
curl -s --noproxy '*' -H "X-Brain-Confirm: 1" -X POST "http://127.0.0.1:$port/api/workspace/import?descriptor=$(python3 -c 'import urllib.parse,sys;print(urllib.parse.quote(sys.argv[1]))' "$BRAIN_FACTORY_TMP/desc.json")&target=$(python3 -c 'import urllib.parse,sys;print(urllib.parse.quote(sys.argv[1]))' "$target_dir")" | python3 -c "
import sys,json; d=json.load(sys.stdin)
...
JSON
curl -s --noproxy '*' -H "X-Brain-Confirm: 1" -X POST "http://127.0.0.1:$port/api/workspace/import?descriptor=$(python3 -c 'import urllib.parse,sys;print(urllib.parse.quote(sys.argv[1]))' "$inferred_dir/project.json")" | python3 -c "
import sys,json; d=json.load(sys.stdin)
