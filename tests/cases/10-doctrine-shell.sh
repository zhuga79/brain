...
brain-doctrine list | grep -q "tax-boundaries" || { echo "FAILED: brain-doctrine list missing tax-boundaries"; exit 1; }
brain-doctrine show tax-boundaries | grep -q "4 этапа\|ЭТАП\|accountant" || { echo "FAILED: brain-doctrine show content missing"; exit 1; }
brain-doctrine search "accountant" | grep -q "tax-boundaries" || { echo "FAILED: brain-doctrine search not finding tax-boundaries"; exit 1; }
...
brain-shell --once status | grep -q "Brain:" || { echo "FAILED: brain-shell --once status missing Brain header"; exit 1; }
...
brain-shell --once "search rarebrainterm" | grep -q "wiki/search-topic.md" || { echo "FAILED: brain-shell search did not find search-topic"; exit 1; }
...
echo "" | brain-shell --once "take $_shell_wid --as smoke-agent" 2>/dev/null | grep -q "Aborted" || { echo "FAILED: shell take without --yes should print Aborted"; exit 1; }
...
brain-shell --yes --once "take $_shell_wid --as smoke-agent" 2>/dev/null | grep -qiE "took|took $_shell_wid" || { echo "FAILED: shell take --yes did not succeed"; exit 1; }
...
echo "" | brain-shell --once "complete $_shell_wid --as smoke-agent" 2>/dev/null | grep -q "Aborted" || { echo "FAILED: shell complete without --yes should print Aborted"; exit 1; }
# Still in-progress
...
brain-shell --help | grep -q "\-\-yes" || { echo "FAILED: --yes not in brain-shell --help"; exit 1; }
